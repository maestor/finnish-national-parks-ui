...
- **Admin view** — Adding and editing visits, including excluding individual trip visits from route calculation, editing park details, hiding parks from the catalog via `/hallinta`, and generating both year reviews and shorter named date-range review share pages for moments like an active month or summer vacation. Super admins can manage admin roles and invitations at `/hallinta/kayttajat`.
