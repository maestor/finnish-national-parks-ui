...
- Public trip detail pages also use `cache: "no-store"` for the same reason until the backend serves stable public asset URLs for trip and visit media. Their same-origin visit-details proxy and its park-visit reads are also private/no-store, so every expansion receives fresh signed image URLs. Their visit image galleries now load on demand when a visitor expands a visit instead of prefetching every deferred trip image on initial page render.
