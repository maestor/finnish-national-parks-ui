...
import type {
  ...
  DateRangeReviewPublishInfo,
  DateRangeReviewPublishResponse,
} from "@/lib/date-range-review";
import { createPathWithSearchParams } from "@/lib/routes";

interface DateRangeReviewPublishControlsProps {
  overview: DateRangeReviewOverview;
  publishInfo: DateRangeReviewPublishInfo;
...
export const DateRangeReviewPublishControls = ({
  overview,
  publishInfo,
  status,
  ...
  const handleUnpublish = () => {
    ...
    startTransition(async () => {
      try {
        await apiFetch(
          createPathWithSearchParams("/api/date-range-review/publish", { name: overview.name }),
          {
            method: "DELETE",
          },
        );
